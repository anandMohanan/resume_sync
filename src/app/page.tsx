  ...
  return (
    <>
      <div className="lg:relative">
        ...
        <div className="lg:sticky top-0 lg:h-screen flex flex-col items-center justify-center mt-10 lg:-mt-4 ">
        <Badge variant="default" className="bg-red-200"> Alpha Out Now ✨ </Badge>
          <h2 className="lg:text-8xl  text-2xl font-bold">Resume Sync</h2>
          <p className=" text-center text-xl p-10">
            Organize resumes with version control, track applications, and join
            a supportive community for career insights.
          </p>
          {!authenticated && (
        ...
        <div className="sticky top-0 h-screen flex flex-col items-center justify-center bg-background  ">
          {!authenticated ? (
            <LoginLink className="text-3xl underline hover:decoration-wavy flex sm:items-center">
              Ready to take control of your professional journey?{" "}
            </LoginLink>
          ) : (
            <Link
              href={`/resume/${user?.id}`}
              className="text-3xl underline hover:decoration-wavy flex"
            >
              Ready to take control of your professional journey?{" "}
            </Link>
          )}
